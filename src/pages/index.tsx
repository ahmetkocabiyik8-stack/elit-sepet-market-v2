...
import { Search, Plus, Minus, ShoppingBag, Trash2, X, MessageCircle, Phone, User, KeyRound, CreditCard, Wallet, Lock, Heart, Play, ShieldCheck, Home, LayoutGrid, Volume2, VolumeX } from "lucide-react";
...
function CertifiedProductBox({ media, title, showCerts, onOpen }: { media: PromoMedia; title: string; showCerts: boolean; onOpen: () => void }) {
  ...
  const toggleSound = (e: React.MouseEvent) => {
    ...
    v.muted = nextMuted;
    if (!nextMuted) {
      // Bazı tarayıcılar sesli oynatmayı kullanıcı etkileşimi olmadan engelliyor;
      // burada zaten bir tıklama olduğu için play() güvenle çağrılabilir.
      v.play().catch(() => {});
    }
    setMuted(nextMuted);
  };
  ...
  return (
    <Wrapper
      ...
      className={`group relative flex w-[220px] shrink-0 flex-col overflow-hidden rounded-2xl border border-border bg-card text-left shadow-[var(--shadow-soft)] transition sm:w-[250px] ${showCerts ? "hover:-translate-y-0.5" : ""}`}
    >
      ...
      {media.type === "video" ? (
        <div className="relative w-full shrink-0 overflow-hidden bg-black" style={{ aspectRatio: "4 / 3" }}>
          <video
            src={media.src}
            className="absolute inset-0 h-full w-full scale-125 object-cover object-center blur-lg opacity-70"
            autoPlay
            muted
            loop
            playsInline
          ...
          <button
            ...
            className="absolute right-2.5 bottom-2.5 z-10 grid h-8 w-8 place-items-center rounded-full bg-black/60 text-white shadow-md transition hover:bg-black/75"
          >
        ...
        <div className="relative w-full shrink-0 overflow-hidden bg-black" style={{ aspectRatio: "4 / 3" }}>
          <img src={media.src} alt="" className="absolute inset-0 h-full w-full scale-125 object-cover object-center blur-lg opacity-70" aria-hidden="true" />
          <img src={media.src} alt="Ürün" className="relative h-full w-full object-contain" loading="lazy" />
        </div>
      )}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-2.5 flex-1">
                      ...
                      <h3 className="mt-1.5 font-sans text-[15px] font-bold leading-snug text-foreground line-clamp-2">
                        {p.name}
                      </h3>
                    </div>

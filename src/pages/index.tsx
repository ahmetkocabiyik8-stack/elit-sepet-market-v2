      ...
      {media.type === "video" ? (
        <>
          <video
            ref={videoRef}
            src={media.src}
            className="h-[68%] w-full object-cover object-top bg-black"
            autoPlay
            muted={muted}
            loop
            playsInline
          ...
          <button
            ...
            className="absolute right-2.5 top-[calc(68%-38px)] z-10 grid h-8 w-8 place-items-center rounded-full bg-black/60 text-white shadow-md transition hover:bg-black/75"
          >
            {muted ? <VolumeX className="h-4 w-4" /> : <Volume2 className="h-4 w-4" />}
          </button>
        </>
      ) : (
        <img src={media.src} alt="Ürün" className="h-[68%] w-full object-contain bg-white p-2" loading="lazy" />
      )}
